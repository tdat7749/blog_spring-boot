...
import { combineLatest, map, Observable, retry, Subject, switchMap, takeUntil, tap } from "rxjs";
...
export class CommentComponent implements OnInit, OnDestroy {
    ...
    onCreateComment() {
        ...
        if (this.createCommentForm.valid) {
            ...
            this.isLoading = true
            this.commentService.createComment(data, this.postId).pipe(takeUntil(this.destroy$)).subscribe({
                next: (response) => {
                    this.messageService.add({
                        severity: "success",
    ...
    onEditComment(comment: Comment, form: NgForm, parentId?: number) {
        ...
        if (form.valid) {
            ...
            this.isLoading = true
            this.commentService.editComment(data).pipe(takeUntil(this.destroy$)).subscribe({
                next: (response) => {
                    this.messageService.add({
                        severity: "success",
                        detail: response.message,
